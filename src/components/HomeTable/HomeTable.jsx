...
export default function HomeTable({ data, selected }) {
    ...
    return (
        <>
            ...
            {
                data.map(level => (
                    <div key={level.id} className={level.risk == selected ? 'home__table_item row selected' : 'home__table_item row'}>
                        <div> {level.risk}</div>
                        {level.categories.map((categorie, index) => (<div key={index}>{categorie.value}</div>))}
                    </div>
                ))
            }
        </>
